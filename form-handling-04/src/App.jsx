        ...
        allTask.map((elem, index) => {
          return <div key={index} className='flex w-1/3 gap-3 py-2 items-center justify-between'>
            ...
            <div className='flex gap-4'>
              <button onClick={() => { deleteTask(index) }} className='bg-red-500 px-2 py-1 cursor-pointer rounded-lg font-bold active:scale-95'>Delete</button>
              <button onClick={() => { editTask(index) }} className='bg-emerald-500 px-3 py-1 cursor-pointer rounded-lg font-bold active:scale-90'>Edit</button>
            </div>
          </div>
        })
      }
